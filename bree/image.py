...
class BaseImage:
    ...
    def find(
        self, needle: Union[str, "BaseImage", Iterable[Union[str, "BaseImage"]]], confidence: Optional[float] = None,
        text_kwargs: Optional[Mapping[str, Any]] = None,
        image_kwargs: Optional[Mapping[str, Any]] = None,
    ) -> Optional["MatchedRegionInImage"]:
        """
        ...
        """
        result = self.find_all(needle, *([confidence] if confidence is not None else []), text_kwargs=text_kwargs, image_kwargs=image_kwargs)
        result = sorted(result, key=lambda res: res.confidence, reverse=True)
        if result:
            return result[0]
        return None
